...
// ─── Live generated requirement: enhanced source → design → code → history trace ───
function LiveRequirementPanel({
  ...
  const primaryConflict = req.conflicts[0]
  const hasConflict = req.conflicts.length > 0
  const status = hasConflict ? "contradicted" : req.status
  const statusCfg = STATUS_CONFIG[status]
  const decision =
    primaryConflict?.fix ||
    req.detail ||
    "No decision has been recorded for this requirement yet."
  const copyQuote = async () => {
    try {
      await navigator.clipboard.writeText(req.sourceQuote)
      setCopied(true)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className="mono"
          style={st({ fontSize: 12.5, fontWeight: 700, color: "var(--t2)" })}
        >
          {req.id}
        </span>
        <span
        ...
        {hasConflict && (
          <span
            style={st({
              ...
              fontWeight: 600,
              color: "var(--warn)",
              background: "rgba(230,163,60,0.12)",
              padding: "3px 10px",
              borderRadius: 100,
            })}
          >
        ...
        <div style={st({ marginLeft: "auto", display: "flex", gap: 8 })}>
          ...
          <Btn v="ghost" onClick={copyQuote}>
            <Ico n={copied ? "check" : "copy"} s={13} c="var(--t2)" />{" "}
      ...
      <div style={st({ flex: 1, overflowY: "auto" })}>
        ...
        >
          ...
          >
            ...
            <ChainStep n={1} label="Source" status="in-sync">
              <div
                style={st({
                  display: "flex",
              ...
              <div style={st({ position: "relative" })}>
                ...
                >
                  <Ico n={copied ? "check" : "copy"} s={11} c="currentColor" />{" "}
                  {copied ? "Copied" : "Copy"}
                </button>
              </div>
              <div
              ...
              >
                ...
                <span style={st({ fontSize: 12, color: "var(--t3)" })}>
                  Quoted by{" "}
                  <strong style={st({ fontWeight: 600, color: "var(--t2)" })}>
                    {req.author}
                  </strong>
                </span>
              </div>
            ...
            <ChainStep n={2} label="Design" status="unlinked">
              <div
                style={st({
                  border: "1px solid var(--bd)",
                  borderRadius: 10,
              ...
              >
                <div
                  style={st({ display: "flex", alignItems: "center", gap: 12 })}
                >
                  <Ico n="figma" s={20} c="var(--t3)" />
                  <div>
                    <div
                      style={st({
                        fontSize: 13,
                    ...
                    >
                      Link the approved Figma frame to verify that the UI
                ...
                >
                  <Ico n="link" s={12} c="currentColor" /> Link design frame
                </button>
              </div>
            </ChainStep>
            ...
            <ChainStep
              n={3}
              label="Code"
              status={hasConflict ? "contradicted" : "unlinked"}
            >
              <div
                style={st({
                  padding: "14px 16px",
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {hasConflict
                        ? "Resolve the evidence conflict before treating a PR as verified."
                        : "Link a GitHub pull request or commit to complete this chain."}
                    </div>
                  </div>
                </div>
              </div>
              {primaryConflict && (
                <div
                  style={st({
                    padding: "12px 16px",
                    background: "rgba(224,95,106,0.07)",
                    border: "1px solid rgba(224,95,106,0.25)",
                    borderRadius: 10,
                    display: "flex",
                    gap: 10,
                  })}
                >
                  ...
                  <div>
                    ...
                    >
                      Contradiction detected · {primaryConflict.id}
                    </div>
                    <div
                      style={st({
                        fontSize: 12.5,
                    ...
                    >
                      <strong>Suggested decision:</strong> {primaryConflict.fix}
                    </div>
                  </div>
                </div>
              )}
            ...
            <div style={st({ display: "flex", gap: 0 })}>
              ...
              <div style={st({ flex: 1, paddingLeft: 16 })}>
                ...
                >
                  ...
                  >
                    Created from {sourceTitle || "captured source"} —
                    requirement extracted by Trace.
                  </span>
                  <span
                    style={st({
                      fontSize: 11,
                ...
                {hasConflict && (
                  ...
                  >
                    ...
                    >
                      Conflict flagged — decision required before this chain can
                      be complete.
                    </span>
                    <span
                      style={st({
          ...
          <aside
            style={st({ padding: "40px 28px 80px", background: "var(--sf)" })}
          >
            ...
            <div style={st({ marginBottom: 36 })}>
              ...
              <div
                style={st({ display: "flex", flexDirection: "column", gap: 6 })}
              >
                <button
                  onClick={copyQuote}
                  style={st({
                ...
                >
                  <Ico n="copy" s={13} c="var(--t2)" /> Copy source quote
                </button>
                <button
                  style={st({
                    display: "flex",
                ...
                >
                  <Ico n="download" s={13} c="var(--t2)" /> Export to spec
                  format
                </button>
                <button
                  style={st({
            ...
            <div>
              ...
              {[
                ...
                { label: "Design", status: "unlinked" as const },
                {
                  label: "Code",
                  status: (hasConflict
                    ? "contradicted"
                    : "unlinked") as "contradicted" | "unlinked",
                },
              ].map(({ label, status: itemStatus }) => {
                const cfg = STATUS_CONFIG[itemStatus]
                return (
                  <div
                    key={label}
                    style={st({
                  ...
                  >
                    <span style={st({ fontSize: 12.5, color: "var(--t2)" })}>
                      {label}
                    </span>
                    <span
                      style={st({
                    ...
                    >
                      {itemStatus === "unlinked" && label === "Design"
                        ? "Not linked"
                        : cfg.label}
                    </span>
                  </div>
...
export function RequirementView() {
  ...
  const liveReq =
    liveBRD?.requirements.find((r) => r.id === activeReqId) ?? null
  if (liveReq) {
    return (
  ...
  const req =
    REQUIREMENTS_DETAIL.find((r) => r.id === activeReqId) ||
    REQUIREMENTS_DETAIL[0]
  ...
  const sourceAuthor = TEAM_DATA.find(
    (t) => t.id === req.source.origin.authorId,
  )
  const figmaApprover = req.figma
    ? TEAM_DATA.find((t) => t.id === req.figma!.approvedById)
    : null
  const codeAuthor = req.code
    ? TEAM_DATA.find((t) => t.id === req.code!.authorId)
    : null
        ...
        >
          ...
          >
            ...
            <ChainStep n={1} label="Source">
              ...
              >
                ...
                {req.source.origin.channel && (
                  <span
                    className="mono"
                    style={st({ fontSize: 12, color: "var(--t3)" })}
                  >
                    {req.source.origin.channel}
                  </span>
                )}
            ...
            <ChainStep
              n={2}
              label="Design"
              status={req.figma?.status || "unlinked"}
            >
              {req.figma ? (
                <div>
                  ...
                  <div
                    style={st({
                      ...
                      borderRadius: 10,
                      background:
                        "linear-gradient(135deg, #F24E1E22, #A259FF22)",
                      border: "1px solid var(--bd2)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                ...
                <div
                  style={st({ display: "flex", alignItems: "center", gap: 10 })}
                >
                  <span style={st({ fontSize: 13, color: "var(--t3)" })}>
            ...
            <ChainStep
              n={3}
              label="Code"
              status={req.code?.status || "unlinked"}
            >
              {req.code ? (
                <div>
                  <div
                        ...
                        >
                          <span
                            style={st({
                              fontSize: 11,
                              fontWeight: 600,
                              color: req.code.merged
                                ? "var(--ok)"
                                : "var(--warn)",
                              background: req.code.merged
                                ? "rgba(78,173,121,0.12)"
                                : "rgba(230,163,60,0.12)",
                              padding: "1px 7px",
                              borderRadius: 100,
                            })}
                          >
          ...
          {/* Sidebar */}
          <div
            style={st({ padding: "44px 28px 80px", background: "var(--sf)" })}
          >
            {/* Decision */}
            ...
            <div>
              ...
              {[
                ...
                {
                  label: "Design",
                  status: (req.figma?.status ||
                    "unlinked") as "in-sync" | "stale" | "contradicted" | "unlinked",
                },
                {
                  label: "Code",
                  status: (req.code?.status ||
                    "unlinked") as "in-sync" | "stale" | "contradicted" | "unlinked",
                },
              ].map(({ label, status }) => {
